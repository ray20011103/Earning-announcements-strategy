...
def load_data_final():
    ...
    # 統一日期格式 (Mixed format for 2015/2/10)
...
def run_parameter_test(df_price, df_ann, holding_days=60, entry_type='Open', max_pos=20):
    ...
    # Match dates to trading calendar
    ...
    for _, row in valid_signals.iterrows():
        ...
        try:
            # Trend Filter
            ma20 = price_ma20.at[row['buy_date'], row['symbol']]
            close = price_close.at[row['buy_date'], row['symbol']]
            if close > ma20:
                to_val = price_turnover.at[row['buy_date'], row['symbol']]
...
def run_robustness_suite():
    ...
    # Plot
    pd.Series(results).plot(kind='bar', color='teal', figsize=(10, 6), title='Robustness: Total Return Comparison')
    plt.ylabel('Total Return (%)')
    plt.grid(True, alpha=0.3)
    plt.savefig('analysis_results/plots/robustness_final_v5.png')
    print("Final Robustness Report saved: analysis_results/plots/robustness_final_v5.png")
